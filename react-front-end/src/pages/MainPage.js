...
export default function MainPage() {
  ...
  useEffect(() => {
    Promise.all([
      Promise.resolve(
        axios.get("/api/mechanics").then((res) => {
      ...
      Promise.resolve(
        axios.get("/api/inspections").then((res) => {
      ...
      if (inspectionID) {
        all[3].forEach((inspection) => {
          if (
            inspection.id === Number(inspectionID) &&
    ...
    const socket = io("wss://fix-it-backend.herokuapp.com");

    socket.on("mechanics", function (data) {
      setMechanics(data);
    });
    socket.on("inspections", function (data) {
      setInspections(data);
  ...
  const setRating = (data) => {
    axios.post("/api/set-rating", data).then((res) => {
  ...
  const userInspectionRequest = (data) => {
    ...
    axios
      .post("/api/new-inspections", userData)
      .then((response) => {
        sessionStorage.setItem("inspectionId", response.data.response[0].id);
        sessionStorage.setItem(
          "mechanicId",
          response.data.response[0].mechanic_id
        );
        ...
        {mode === REQUEST && (
          ...
          >
            <OrderRequest
              ...
              userInspectionRequest={userInspectionRequest}
              mechanic={mechanic}
              onCancel={() => back()}
            />
          </Suspense>
